Income <- structure(list(X = 1:30, Education = c(10, 10.4013377926421, 
                                                 ...
                                                 20.7558528428094, 21.1571906354515, 21.5986622073579, 22), Income = c(26.6588387834389, 
                                                                                                                       ...
                                                                                                                       80.2605705009016)), .Names = c("X", "x", "y"), class = "data.frame", row.names = c(NA, 
                                                                                                                                                                                                                       -30L))
require("ggplot2")

mod <- lm(y ~ x, data = Income)
...
wzrost <- c(161, 184, 148, 196, 138, 166, 189, 173, 162, 171)
waga <- c(63, 81, 56, 91, 57, 67, 76, 72, 62, 68)
ludzie <- as.data.frame(cbind(wzrost, waga))
summary(ludzie)
model <- lm(waga~wzrost, data= ludzie)
summary(model)
plot(waga,wzrost,col = "blue",main = "model regresji wzrostu i wagi",
     abline(lm(wzrost~waga)),cex = 1.3,pch = 16,xlab = "Waga w Kg",ylab = "wzrost w cm")

par(mfrow=c(1, 2))
plot(density(wzrost), main="Wykres gêstoœci- wzrost", ylab="Czêstotliwoœæ", xlab="wzrost")
polygon(density(wzrost), col="blue")
plot(density(waga), main="Wykres gêstoœci- waga", ylab="Czêstotliwoœæ", xlab="waga")
polygon(density(waga), col="red")


par(mfrow=c(1, 2))  # divide graph area in 2 columns
boxplot(wzrost, data= ludzie, main="wzrost") # box plot for 'speed'
boxplot(waga, data= ludzie, main= "waga")
